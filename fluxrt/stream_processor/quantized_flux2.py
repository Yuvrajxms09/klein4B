# Cloned from https://huggingface.co/aydin99/FLUX.2-klein-4B-int8/blob/main/quantized_flux2.py
...
class QuantizedDiffusersModel(ModelHubMixin):
    ...
    def from_pretrained(
        ...
        if not is_accelerate_available():
            raise ValueError(
                "Reloading a quantized diffusers model requires the accelerate library."
            )
        ...
        if not os.path.exists(model_config_path):
            raise ValueError(
                f"{CONFIG_NAME} not found in {pretrained_model_name_or_path}."
            )
        ...
        if os.path.exists(checkpoint_file):
            ...
            if not os.path.exists(checkpoint_file):
                raise ValueError(
                    f"No safetensor weights found in {pretrained_model_name_or_path}."
                )
